...
JWT_TOKEN=$(curl -s -u ${USERNAME}:${PASSWORD} http://localhost:9021/api/metadata/security/1.0/authenticate | jq -r .auth_token)
...
echo -e "\nRename the cluster in Control Center from ${clusterId} to Kafka Kitabikin"
curl -X PATCH -H "Authorization: Bearer ${JWT_TOKEN}" -H "Content-Type: application/merge-patch+json" -d '{"displayName":"Kafka Kitabikin"}' http://localhost:9021/2.0/clusters/kafka/$clusterId
