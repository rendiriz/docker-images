...
# Generate CA key
openssl req -new -x509 -keyout snakeoil-wj-1.key -out snakeoil-wj-1.crt -days 365 -subj '/CN=wj1.test.kitabikin.com/OU=TEST/O=KITABIKIN/L=Bandung/ST=WJ/C=ID' -passin pass:${SSL_PASSWORD} -passout pass:${SSL_PASSWORD}
...
printf '%s\0' "${users[@]}" | xargs -0 -I{} -n1 -P15 sh -c 'SSL_PASSWORD=$SSL_PASSWORD ./certs-create-per-user.sh "$1" > "certs-create-$1.log" 2>&1 && echo "Created certificates for $1"' -- {}
