...
openssl x509 -req -CA ${CA_PATH}/snakeoil-wj-1.crt -CAkey ${CA_PATH}/snakeoil-wj-1.key -in $i.csr -out $i-wj1-signed.crt -sha256 -days 365 -set_serial ${CERT_SERIAL} -passin pass:confluent -extensions v3_req -extfile <(cat <<EOF
[req]
...
keytool -noprompt -keystore kafka.$i.keystore.jks -alias snakeoil-caroot -import -file ${CA_PATH}/snakeoil-wj-1.crt -storepass confluent -keypass confluent
...
keytool -noprompt -keystore kafka.$i.truststore.jks -alias snakeoil-caroot -import -file ${CA_PATH}/snakeoil-wj-1.crt -storepass confluent -keypass confluent
